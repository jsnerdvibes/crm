...
export class AuthService {
  ...
  async login(dto: LoginDTO) {
    ...
    await this.repo.saveRefreshToken(user.id, refreshToken, new Date(Date.now() + parseDuration(config.jwt.expiresIn)));
  ...
  async refreshAccessToken(refreshToken: string) {
    ...
    const newRefreshToken = jwt.sign({ sub: user.id }, config.jwt.refreshSecret, { expiresIn: "1h" });
    await this.repo.saveRefreshToken(user.id, newRefreshToken, new Date(Date.now() + parseDuration(config.jwt.expiresIn)));
