...
export class AuthService {
  ...
  async register(data: { tenantName: string; email: string; password: string }) {
    ...
    // Hash password
    const passwordHash = await bcrypt.hash(data.password, Number(process.env.BCRYPT_SALT_ROUNDS));
