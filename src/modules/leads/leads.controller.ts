 ...
 * /api/v1/leads:
 *   get:
 *     summary: Get all leads within the tenant (supports pagination)
 *     tags: [Leads]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 */
  ...
  findAll = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await this.service.getAllLeads(tenantId, query);

      return res.json(successResponse('Leads fetched successfully', result));
    } catch (error) {
      next(error);
    }
  };
