...
async function testDb() {
    ...
    console.log('Tenancy Mode:', config_1.config.tenancy.mode, '\n');
    // ------------------------------------
    // 1️⃣ Test default prisma (no tenancy)
    // ------------------------------------
    try {
        console.log('➡ Testing base prisma…');
        const result = await db_1.prisma.$queryRawUnsafe('SELECT 1 as ok;');
    ...
    try {
        console.log("\n➡ Testing db('tenant1')…");
        const tenantClient = (0, db_1.db)('tenant1');
    ...
    // ------------------------------------
    if (config_1.config.tenancy.mode === 'schema') {
        try {
            console.log('\n➡ Testing db() without tenantId in schema mode…');
            (0, db_1.db)(); // should throw
        }
        catch (err) {
    ...
    // ------------------------------------
    if (config_1.config.tenancy.mode === 'field') {
        try {
            console.log('\n➡ Testing Field Mode tenantId injection…');
            const tenantClient = (0, db_1.db)('tenantABC');
            const result = await tenantClient.user.findMany({
                where: { email: 'test@example.com' },
            });
            console.log('Field Mode Query Result:');
            console.log(result);
            console.log('(If working: this query was forced where tenantId = tenantABC)');
        }
        catch (err) {
